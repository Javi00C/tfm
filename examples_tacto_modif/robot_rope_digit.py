...
from pybullet_env_classes.robot_modif import UR5Robotiq85  # Import the robot class
...
class RopeRobotSimulation:
    ...
    def _load_digit_sensor(self):
        ...
        # Pass cfg and digits to the robot so it can attach the DIGIT sensor
        self.robot.setup_digit(self.cfg, self.digits)   
    ...
    def apply_action(self, action):
        ...
        if len(action) == 6:
            self.robot.move_ee_vel(action, control_method='end')
        else:
            self.robot.move_ee_vel(action, control_method='joint')
...
if __name__ == "__main__":
    ...
    sim.apply_action((0.1, 0, 0, 0, 0, 0))#((0.4, 0.0, 0.5, 0, 0, 0))
    for _ in range(100):
        sim._step_simulation()
        time.sleep(1/240)

    # sim.apply_action((0.01, 0, 0, 0, 0, 0))#((0.4, 0.0, 0.5, 0, 0, 0))
    # for _ in range(100):
    #     sim._step_simulation()
    #     time.sleep(1/240)

    # sim.apply_action((0, 0.01, 0, 0, 0, 0))#((0.4, 0.0, 0.5, 0, 0, 0))
    # for _ in range(100):
    #     sim._step_simulation()
    #     time.sleep(1/240)
    ...
    for step in range(24000):  # ~10 seconds at 240Hz
        #print(sim.get_observation()['ee_pos'])
