...
class UR5Sim:
    def __init__(self,
        ...
        self.save_file = "reachable_goals.csv"
        self.file = open(self.save_file, mode='a', newline='')
    ...
    def reset(self):
        ...
        # Save pose if reachable
        if dist_pose_rchd <= 0.02:
            self.writer.writerow(target.tolist())  # Convert numpy array to list
            print(f"Pose saved: {target}")
        #self.set_joint_angles(joint_angles)
        for i in range(200):
            pybullet.stepSimulation()
