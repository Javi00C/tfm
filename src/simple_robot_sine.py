...
class SimpleRobotEnvSine(gym.Env):
    ...
    def reset(self, *, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        # Draw a sine wave in the middle of the environment
        for y in range(self.size):
            x = int((self.size // 2) + 50 * np.sin(2 * np.pi * y / 100))  # Sine wave with amplitude 50 and period 100
            self.full_state[y, :x] = (255, 255, 255)  # Draw white pixels to the left of the sine wave
